    ...
    public class PlayerController : MonoBehaviour, IPlayerController, IPhysicsObject
    {
        ...
        private void Update()
        {
            ...
            if (swordInhand)
            {
                if (_lookLock)
                {
                    // ANI: Player with sword; aim animation
                }
                else
                {
            ...
            {
                // ANI: Player holding nothing; idle animation
            }
        }
        ...
        private void Attack()
        {
            if (swordInhand)
            {
                if (_lookLock)
                {
                 // Also have the sword spawn infront of the player instead of on top. And set a 2 second timer for how long the sword is out before despawning
      

                //Note for Harrish: when sword is thrown, change player sprite so that the player is not holding sword for 2 seconds.
                    swordInhand = false;
